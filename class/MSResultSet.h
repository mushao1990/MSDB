...
/** Represents the results of executing a query on an `<MSDatabase>`.
...
@interface MSResultSet : NSObject {
    MSDatabase          *_parentDB;
    MSStatement         *_statement;
...
/** `MSStatement` used by result set. */

@property (atomic, retain) MSStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<MSStatement>`
 
 @param statement A `<MSStatement>` to be performed
 
 @param aDB A `<MSDatabase>` to be used
 
 @return A `MSResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(MSStatement *)statement usingParentDatabase:(MSDatabase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(MSDatabase *)newDb;
