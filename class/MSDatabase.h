...
#import "MSDatabasePool.h"
    ...
    #define MSDBAutorelease(__v) ([__v autorelease]);
    #define MSDBReturnAutoreleased MSDBAutorelease

    #define MSDBRetain(__v) ([__v retain]);
    #define MSDBReturnRetained MSDBRetain

    #define MSDBRelease(__v) ([__v release]);

    #define MSDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define MSDBAutorelease(__v)
    #define MSDBReturnAutoreleased(__v) (__v)

    #define MSDBRetain(__v)
    #define MSDBReturnRetained(__v) (__v)

    #define MSDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define MSDBDispatchQueueRelease(__v)
    #else
        #define MSDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^MSDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in MSDB are:

 - `MSDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<MSResultSet>` - Represents the results of executing a query on an `MSDatabase`.
 - `<MSDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<MSDatabasePool>` - A pool of `MSDatabase` objects.
 - `<MSStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [MSDB on GitHub](https://github.com/ccgus/MSDB) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [MSDB mailing list](http://groups.google.com/group/MSDB)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `MSDatabase` object and use it across multiple threads. Instead, use `<MSDatabaseQueue>`.
...
/** Create a `MSDatabase` object.
 
 An `MSDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `MSDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `MSDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    MSDatabase *db = [MSDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    MSDatabase *db     = [MSDatabase databaseWithPath:dbPath];
 ...
 @return `MSDatabase` object if successful; `nil` if failure.

 */

+ (instancetype)databaseWithPath:(NSString*)inPath;

/** Initialize a `MSDatabase` object.
...
/** Execute single update statement
 ...
 @note If you want to use this from Swift, please note that you must include `MSDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeUpdate:withArgumentsInArray:>`.
 */
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(MSDBExecuteStatementsCallbackBlock)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<MSResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[MSResultSet next]>`) from one record to the other.
 ...
 @return A `<MSResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see MSResultSet
 @see [`MSResultSet next`](<[MSResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
 
 @note If you want to use this from Swift, please note that you must include `MSDatabaseVariadic.swift` in your project. Without that, you cannot use this method directly, and instead have to use methods such as `<executeQuery:withArgumentsInArray:>`.
 */

- (MSResultSet *)executeQuery:(NSString*)sql, ...;
...
- (MSResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (MSResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
- (MSResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (MSResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(MSDatabase *adb) {
        ...
        int rowCount = 0;
        MSResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        MSDBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [MSDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one MSDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using MSDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to MSDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using MSDB you will not need to interact directly with `MSStatement`, but rather with `<MSDatabase>` and `<MSResultSet>` only.
...
@interface MSStatement : NSObject {
