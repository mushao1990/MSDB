...
+ (instancetype)resultSetWithStatement:(MSStatement *)statement usingParentDatabase:(MSDatabase*)aDB {
    ...
    return MSDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    MSDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return MSDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"MSDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
